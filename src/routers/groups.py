...
def get_group_balances(
  ...
  # Разрешаем смотреть даже для soft-deleted/archived, но только участникам
  ...
  group = get_group_incl_deleted_or_404(db, group_id)  # <— важное изменение
...
def get_groups_for_user(
  ...
  if sb == "name":
      ...
      order_clauses = [getattr(Group.id, sd)()]
...
def group_detail(
  ...
  # ВАЖНО: разрешаем получать detail даже для soft-deleted/archived
  group = get_group_incl_deleted_or_404(db, group_id)

  # Проверка членства без запрета на удалённые группы
...
def archive_group(
  ...
  if group.status == GroupStatus.archived:
      return
  # Нельзя архивировать удалённую (soft) группу
  if group.deleted_at is not None:
      raise HTTPException(status_code=409, detail="Удалённую группу нельзя архивировать")
...
def soft_delete_group(
  ...
  """
  Удаление:
    • архивную группу удалять НЕЛЬЗЯ (ни soft, ни hard);
  ...
  # Запрет: архивную нельзя удалять
  if group.status == GroupStatus.archived:
      raise HTTPException(status_code=409, detail="Архивированную группу нельзя удалять")
...
def delete_preview(
  ...
  Возвращает режим удаления для фронта:
    - forbidden: есть долги ИЛИ группа архивирована — удаление запрещено;
    ...
    - disabled: группа уже soft-deleted.
  """
  group = get_group_incl_deleted_or_404(db, group_id)
  if group.owner_id != current_user.id:
      raise HTTPException(status_code=403, detail="Only owner can perform this action")
  ...
  # Архивированную удалять нельзя
  if group.status == GroupStatus.archived:
      return {"mode": "forbidden", "has_debts": False, "has_transactions": _has_active_transactions(db, group_id)}
