...
def create_transaction(
    ...
    # 5) Склейка дублей shares по user_id (Decimal!); и проверка суммы
...
def update_transaction(
    ...
    """
    Обновить транзакцию:
    - тип менять нельзя (если не совпадает — 409)
    - валидация валюты/категории/членства
    - shares пересоздаются
    """
    tx = (
        db.query(Transaction)
    ...
    # авторизация и активность группы
    require_membership(db, tx.group_id, current_user.id)
    group = guard_mutation_for_member(db, tx.group_id, current_user.id)
    ensure_group_active(group)

    # запрет на смену типа
    ...
    # агрегирование shares и проверка суммы
    aggregated_shares: Dict[int, Dict[str, Decimal | int | None]] = {}
    if patch.shares:
